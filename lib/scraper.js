...
scraper.prototype.scrape = function(cleanName, cleanQuote, callback) {
	...
	request(wikiURL, function(error, response, body) {
		if (!error && response.statusCode == 200) {
			...
			if ($("*:contains(Wikiquote does not have an article with this exact name.)").length = 0) {
				pageExist = true;
				if ($("*:contains("+quoteToFind+")").length > 0) {
					var found = true;
					...
					$("*:contains("+quoteToFind+")").parents().each(function() {
					...
					// do nothing, found initially assigned to false
				}
			}
			else {
				pageExist = false;
	...
	if (false) {
		...
		callback(true, cleanName, cleanQuote, found, bgColor);
	}
	else {
		// everthing went right 
		callback(null, cleanName, cleanQuote, found, bgColor);
	}
}

scraper.prototype.calcAuthenticity = function(cleanName, cleanQuote, found, bgColor, pageExist) {
